...
```{r, echo = FALSE, fig.keep = 'last', fig.align = 'center', message = FALSE, warning = FALSE}

source("./chart3_maya.R")

```

The above figure visualizes the number of crimes committed and reported per month over the years which this dataset spans. I chose this visualization because it effectively visualizes annual crimes trends, such as the number of committed crimes and reported crimes peaking during the summer months. Furthermore, by graphing both the number of committed crimes and the number of reported crimes, it allows us to see how these numbers track with one another. The figure seems to suggest that most crimes are committed and reported within the same month because the lines of both track very closely with one another, although additional analysis would need to take place in order to confirm this trend. 
